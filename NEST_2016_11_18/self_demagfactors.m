% DEMAGNETISATION FACTORS CALCULATION
...
% demagnetising factors are given in Kalezhi et al [2].

%% CHOOSES WHICH SHAPE TO USE
...
function [ nxx nyy nzz ] = self_demagfactors(a, b, t, alpha, islandgeo, demag_tol)
switch islandgeo
    case 1
...
%% TRUNCATED ELLIPTIC CONE DEMAGNETISATION FACTORS CALCULATION
% The demagnetisation field of a uniformly magnetised truncated elliptic
% cone is non-uniform. The integral equations used to calculate
% demagnetising factors were determined using Fourier transform approach
% proposed by Beleggia et al. [2]. The thorough derivation of the
% magnetometric demagnetising factors is given by Kalezhi et al. [1]. For a
% truncated elliptic cone the demagnetisation factors are given by:
% 
% $$N_{xx} = \frac{ab}{2\pi V} \int^\pi_{x = 0} \cos x dx \int^\{2\pi}_{\phi_k = 0} \frac{\cos^2 \phi_k}{\sqrt{cos^2 \phi_k + \beta^{-2}\sin^2\phi_k }} d\phi_k 
%            * \int^t_{z=0} \int^t_{z'=0} \frac{(1 - z/z_0)(1-z'/z_0)dz dz'}{\sqrt{F(z,z',\phi_k,x)}} $$
% 
% where 
%
...
%
% $$N_{yy} = \frac{ab}{2\pi V \beta^2} \int^\pi_{x = 0} \cos x dx \int^\{2\pi}_{\phi_k = 0} \frac{\sin^2 \phi_k}{\sqrt{cos^2 \phi_k + \beta^{-2}\sin^2\phi_k }} d\phi_k 
%            * \int^t_{z=0} \int^t_{z'=0} \frac{(1 - z/z_0)(1-z'/z_0)dz dz'}{\sqrt{F(z,z',\phi_k,x)}} $$
%
% $$N_{zz} = 1 - N_{xx} - N_{yy}$$
%
...
%
% Where z_o = \frac{t}{1 - a_t/a}, 0 <= z <= t, t<= z_0, -a<=x<=a, -b<=y<=b
% and \beta = b/a. t is the height of the truncated cone, a_t is the
...
%% ELLIPTIC CYLINDER DEMAGNETISATION FACTORS CALCULATION
...
%% PRISM DEMAGNETISATION FACTORS CALCULATION
...
%[2] M. Beleggia and M. De Graef, “On the computation of the demagnetization
%tensor field for an arbitrary particle shape using a Fourier space
%approach,”J. Magn. Magn. Mater., vol. 263, pp. L1–L9, Jul. 2003.